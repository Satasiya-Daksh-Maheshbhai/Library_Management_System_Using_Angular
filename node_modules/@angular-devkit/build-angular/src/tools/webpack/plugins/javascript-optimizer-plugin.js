                ...
                try {
                    ...
                    for (const { name, code, map, cacheItem } of scriptsToOptimize) {
                        ...
                        tasks.push(workerPool
                            ...
                            if (errors?.length) {
                                const isSpawnEperm = errors.every((error) => String(error).includes('spawn EPERM'));
                                if (isSpawnEperm) {
                                    const fallbackAsset = map
                                        ? new SourceMapSource(code, name, map)
                                        : new OriginalSource(code, name);
                                    compilation.updateAsset(name, fallbackAsset, (assetInfo) => ({
                                        ...assetInfo,
                                        minimized: false,
                                    }));
                                    return;
                            ...
                            if (String(error?.stack || error?.message || error).includes('spawn EPERM')) {
                                const fallbackAsset = map
                                    ? new SourceMapSource(code, name, map)
                                    : new OriginalSource(code, name);
                                compilation.updateAsset(name, fallbackAsset, (assetInfo) => ({
