...
class CssOptimizerPlugin {
     ...
     */
    async optimize(input, name, inputMap, target) {
        let sourceMapLine;
        if (inputMap) {
        ...
        try {
            return await this.esbuild.transform(sourceMapLine ? input + sourceMapLine : input, {
                loader: 'css',
            ...
            if ((error?.code === 'EPERM') || String(error).includes('spawn EPERM')) {
                return { code: input, warnings: [], map: inputMap };
            }
            throw error;
        }
    }
